...
в ворде, позже перенесу сюда(наверное)
...
![ ](/IMG/5lab1.png)
Если пользователь не авторизован или поле токена пустое, то при попытке добавления новой задачи, результатом будет ошибка: 500 - "Token is no valid"
![ ](/IMG/5lab2.png)
...
Выполнение одного из запросов (get room) с авторизацией

![ ](/IMG/5lab_with_auth.png)

Результат: 200 - json объект с добавленой задачей

#### Анализ flake8

С помощью flake8 просканировала файлы проекта. В файлах были незначительные ошибки в оформлении кода. Наприер в файле питона autorizatin анализатор показал следующие ошибки.
Список ошибок
...
#### Анализ pylint
С помощью pylint просканировала файлы. Во всех файлах, кроме authorization были ошибки в оформлении кода. В файле pylint подсказал, что был использован лишний else
Список ошибок до
